...
#   3. Java client (root Gradle)  -> build/libs                       (needs a JDK)
#   4. IntelliJ RuneScript plugin -> runescript/plugin/build/distributions (needs JDK 21)
#
#   .\build.ps1             # debug rust + wasm + java + plugin
...
# 3. Java client (root Gradle project; assemble = compile + jar, no tests).
if (-not $SkipJava) {
    $jdk = Get-Jdk21Home
...
# 4. IntelliJ RuneScript plugin (separate Gradle project under runescript/).
if (-not $SkipPlugin) {
    $jdk = Get-Jdk21Home
    if ($jdk) {
        $env:JAVA_HOME = $jdk
