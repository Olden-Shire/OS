//! `jagex3.sound.CodeBook` — Vorbis I codebook (Huffman tree + optional VQ lookup).
...
impl CodeBook {
    pub fn decode(br: &mut BitReader<'_>) -> Self {
        br.read_bits(24); // sync pattern, discarded
    ...
    /// Walk the Huffman tree using bits from the reader, returning the entry index.
...
/// Build the Huffman tree (in array form) from a list of code lengths. Mirrors Java's
/// `CodeBook.prepareHuffman` literally — each node either has a non-negative index of the
/// "1" child (the "0" child is implicitly node+1) or a negative `!entry` terminator.
