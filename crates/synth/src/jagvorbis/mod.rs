//! Port of `jagex3.sound.JagVorbis` + dependencies.
//!
//! The rev1 client's Vorbis storage strips standard Ogg framing and stores raw audio
//! packets in a small custom container. All samples in the cache share a single header
//! file (codebooks/floors/residues/mappings/modes), kept here in [`VorbisHeaders`].
...
impl JagVorbis {
    pub fn decode(src: &[u8]) -> Self {
        ...
        for _ in 0..n_packets {
            ...
            buf.copy_from_slice(&p.data[p.pos..p.pos + len as usize]);
            p.pos += len as usize;
