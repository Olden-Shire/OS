...
// @ObfuscatedName("ej.dl(Ljava/lang/String;ZI)V") — Client.gameDrawMain
// Minimap render entry. Java's render2DGround (World.java:914) walks
// the rendered ground tile-by-tile, rasterizing each tile into a 4×4
// block of the 512×512 `Pix32 minimap` buffer using `minimapRgb`
// (QuickGround / Java's var56 = jittered underlay) or per-corner
// `minimapOverlay` / `minimapUnderlay` (Ground / var65). The 512×512
// is then rotated by camera yaw and clipped to a 152×152 ellipse
// before the per-frame chrome blit.
//
// We render directly to the chrome surface: per visible tile we pick
// the underlay or overlay RGB, rotate the tile offset by camera yaw,
// and stamp the destination pixels. Player marker renders as a white
// dot at the centre; minimapState gating + flag rendering is TODO.
pub fn draw_minimap(x: i32, y: i32, w: i32, h: i32) {
    ...
    // Bridge handling: tiles with `mapl[1] & 0x2` have their level-1
    // geometry shifted into level-0's render slot by ClientBuild.
    // The minimap follows the same convention as the 3D viewport:
    // bridges render as the deck (level 1 floor data) instead of the
    // water/ground beneath. Pre-extract the mask so the per-tile
    // closures don't keep a long-lived borrow of State.
    let bridge_mask: Vec<Vec<bool>> = (0..104).map(|bx| {
        (0..104).map(|bz| (state.mapl[1][bx][bz] & 0x2) != 0).collect()
    }).collect();
    let tile_level = |tx: i32, tz: i32| -> usize {
        if tx < 0 || tx >= 104 || tz < 0 || tz >= 104 { return 0; }
        if bridge_mask[tx as usize][tz as usize] { 1 } else { 0 }
    };

    // Jittered HSL → RGB lookup, matching Java's finishBuild lines
    // 870-944 minimap path (var49 / var61 / var65). hueOff and ligOff
    // are global per-build offsets; we apply them to the underlay /
    // overlay HSL fields before the palette lookup so the minimap
    // tone differs subtly per build (Java randomises these per scene).
    let hue_off = MINIMAP_HUE_OFF.load(std::sync::atomic::Ordering::Relaxed);
    let lig_off = MINIMAP_LIG_OFF.load(std::sync::atomic::Ordering::Relaxed);
    let palette = pix3d::colour_table();
    let get_table_local = |hue: i32, mut sat: i32, light: i32| -> i32 {
        if light > 179 { sat /= 2; }
        if light > 192 { sat /= 2; }
        if light > 217 { sat /= 2; }
        if light > 243 { sat /= 2; }
    ...
    let get_u_col_local = |idx: i32, light: i32| -> i32 {
        if idx == -1 { return 12345678; }
        let mut v = ((idx & 0x7F) * light) / 128;
        if v < 2 { v = 2; } else if v > 126 { v = 126; }
        (idx & 0xFF80) + v
    };

    let underlay_rgb = |tx: i32, tz: i32| -> Option<i32> {
        if tx < 0 || tx >= 104 || tz < 0 || tz >= 104 { return None; }
        let lvl = tile_level(tx, tz);
        let t1 = state.floor_t1[lvl][tx as usize][tz as usize] as i32 & 0xFF;
        if t1 == 0 { return None; }
        let fl = flu_type::list(t1 - 1);
        // Java's var49 — getTable(hueOff+hue & 0xFF, sat, clamp(ligOff+light, 0, 255)).
        let h = (hue_off + fl.hue) & 0xFF;
        let l = (lig_off + fl.lightness).clamp(0, 255);
        let idx = get_table_local(h, fl.saturation, l);
        let modulated = get_u_col_local(idx, 96);
        if modulated == 12345678 { return None; }
        Some(palette[(modulated as usize) & 0xFFFF])
    };
    let overlay_rgb = |tx: i32, tz: i32| -> Option<i32> {
        if tx < 0 || tx >= 104 || tz < 0 || tz >= 104 { return None; }
        let lvl = tile_level(tx, tz);
        let t2 = state.floor_t2[lvl][tx as usize][tz as usize] as i32 & 0xFF;
        if t2 == 0 { return None; }
        let fl = flo_type::list((t2 - 1) & 0xFF);
        // Java's mapcolour override (line 933-942) — when set, the
        // overlay uses MAP HSL fields with the same jitter applied to
        // produce var65 = colourTable[getOCol(var68, 96)].
        if fl.mapcolour != -1 {
            let h = (hue_off + fl.map_hue) & 0xFF;
            let l = (lig_off + fl.map_lightness).clamp(0, 255);
            let idx = get_table_local(h, fl.map_saturation, l);
            let modulated = get_u_col_local(idx, 96);
            return Some(palette[(modulated as usize) & 0xFFFF]);
        }
        if fl.colour == 16711935 { return None; }  // 0xFF00FF invisible marker
        // Java's var61 path — jitter the base HSL then get_o_col.
        let h = (hue_off + fl.hue) & 0xFF;
        let l = (lig_off + fl.lightness).clamp(0, 255);
        let idx = get_table_local(h, fl.saturation, l);
        let modulated = get_u_col_local(idx, 96);
    ...
    // Pre-extract a 4-corner tile colour into one merged RGB. Java's
    // Ground stores separate minimapOverlay/minimapUnderlay per shape
    // — for the 1-pixel-per-tile rendering we pick overlay if set,
    // else underlay, else dark grey.
    let tile_colour = |tx: i32, tz: i32| -> i32 {
        if let Some(c) = overlay_rgb(tx, tz) { return c; }
        if let Some(c) = underlay_rgb(tx, tz) { return c; }
        0x202020 // unmapped tile
    };

    // Fill the minimap viewport in the background colour first so any
    // gaps (out-of-bounds, water without colour) read as the OSRS
    // dark frame instead of stale pixels from the previous frame.
    pix2d::fill_rect(x, y, w, h, 0x0a0a0a);

    // The minimap viewport in rev1 is 152×152 with a 73,75 origin
    // offset from its top-left. We render an ellipse-clipped square
    // around the player at the centre.
    let cx = x + w / 2;
    let cy = y + h / 2;
    // Pixels per tile. Java uses 4 pixels per tile in a 512×512
    // image; on the smaller chrome minimap that gets scaled down to
    // roughly 2 pixels per tile. We pick 2 directly to skip the
    // intermediate buffer.
    let pixels_per_tile = 2i32;
    let view_tiles = (w.min(h) / pixels_per_tile / 2).max(8);

    let sin_t = pix3d::sin_table();
    let cos_t = pix3d::cos_table();
    let yaw_idx = (cam_yaw & 0x7FF) as usize;
    let sy = sin_t[yaw_idx];
    let cy_cos = cos_t[yaw_idx];

    // Ellipse mask: drop pixels outside (dx/rx)^2 + (dy/ry)^2 > 1.
    let rx = (w / 2) - 2;
    let ry = (h / 2) - 2;
    let rx2 = (rx * rx).max(1);
    let ry2 = (ry * ry).max(1);

    for dz in -view_tiles..=view_tiles {
        for dx in -view_tiles..=view_tiles {
            let tx = lp_x + dx;
            let tz = lp_z + dz;
            if tx < 0 || tx >= 104 || tz < 0 || tz >= 104 { continue; }
            let rgb = tile_colour(tx, tz);

            // Rotate tile offset by camera yaw so the minimap points
            // toward the camera's forward direction (rev1 chrome spins
            // the minimap with the compass). Convention:
            //   yaw=0 → camera looks NORTH → north tiles render UP
            //   yaw=π/2 (idx 512) → camera looks WEST → west tiles UP
            // Rotation is by -yaw (CW from above), then we flip the
            // Z-component because pixel +Y is screen-down:
            //   sx =  dx*cosθ + dz*sinθ
            //   sz = -dx*sinθ + dz*cosθ
            //   screen = (cx + sx, cy - sz)
            let px_world_x = dx * pixels_per_tile;
            let px_world_z = dz * pixels_per_tile;
            let sx = (px_world_x * cy_cos + px_world_z * sy) >> 16;
            let sz = (-px_world_x * sy + px_world_z * cy_cos) >> 16;
            let rot_x = cx + sx;
            let rot_z = cy - sz;

            let edx = rot_x - cx;
            let edz = rot_z - cy;
            if edx * edx * ry2 + edz * edz * rx2 > rx2 * ry2 { continue; }
            pix2d::fill_rect(rot_x, rot_z, pixels_per_tile, pixels_per_tile, rgb);
        }
    }

    drop(state);
    // Player marker — small white dot at the minimap centre.
    pix2d::fill_rect(cx - 1, cy - 1, 3, 3, 0xFFFFFF);
}
