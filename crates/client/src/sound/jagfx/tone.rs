// @ObfuscatedName("t")
// jag::oldscape::sound::Tone — additive-synthesis sound generator.
//
// Combines up to 5 harmonic oscillators (sine/square/saw/noise) with
// optional amplitude+frequency modulation envelopes, release-gating,
// reverb, and a final IIR filter pass. Used by JagFX to synthesise the
// rev1 client's sound-effect samples.
...
// Shared lookup tables (sine + Java-Random-seeded noise).
struct Tables {
    noise: Vec<i32>,
    sine: Vec<i32>,
}
...
// java.util.Random LCG — emulated for byte-exact noise table match.
struct JavaRandom { seed: u64 }
impl JavaRandom {
    fn new(seed: i64) -> Self { Self { seed: ((seed as u64) ^ 0x5DEEC_E66D) & ((1 << 48) - 1) } }
    fn next(&mut self, bits: u32) -> i32 {
        self.seed = (self.seed.wrapping_mul(0x5DEEC_E66D).wrapping_add(0xB)) & ((1 << 48) - 1);
        (self.seed >> (48 - bits)) as i32
    }
    fn next_int(&mut self) -> i32 { self.next(32) }
}
...
impl Tone {
    // @ObfuscatedName("t.r([III)V") — Tone.generate
    pub fn generate(&mut self, out: &mut [i32], sample_count: usize, length: i32) {
        for v in out.iter_mut().take(sample_count) { *v = 0; }
        if length < 10 { return; }
        ...
        for sample in 0..sample_count {
            let mut frequency = self.frequency_base.gen_next(sample_count as i32);
            ...
            if let (Some(rate), Some(range)) = (&mut self.frequency_mod_rate, &mut self.frequency_mod_range) {
                ...
                // Java's Tone.synth adds the wave-modulated value into
                // the frequency for that sample (FM / vibrato).
                // Previously the result was discarded into `_f`, which
                // muted all Tone-driven FM effects.
                frequency = frequency.wrapping_add(wave_func(freq_phase, g, rate.form) >> 1);
    ...
    // @ObfuscatedName("t.d(Lev;)V") — Tone.load
    pub fn load(&mut self, p: &mut Packet) {
        ...
        // Java loops to 10 but writes to harmonicVolume[var5] which is a
        // 5-slot array — the data never carries more than 5 harmonics so
        // the higher slots are inert. We still consume the bytes the
        // packet promises us in case some pathological record uses them.
        for h in 0..10 {
            let v = p.gsmart();
            if v == 0 { break; }
            if h < 5 {
