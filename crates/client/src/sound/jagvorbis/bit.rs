// @ObfuscatedName("dt") — jag::oldscape::sound::JagVorbis::BitUnpacker
// LSB-first bit reader. Java uses a static buffer; here we keep it
// instance-based so multiple readers can coexist (e.g., header decode +
// packet decode).
...
// @ObfuscatedName("MathTool.bitsRequired") — ceil(log2(n + 1)). Returns 0 for n ≤ 0.
pub fn bits_required(n: i32) -> i32 {
    if n <= 0 { 0 } else { 32 - (n as u32).leading_zeros() as i32 }
}

// @ObfuscatedName("dt.float32Unpack") — JagVorbis custom float32 unpack
